...
<h4>Индивидуальные нормы по всем<br />Среднее качество сигнала на блоке (=Validity)</h4>
<table cellspacing="1" class="tbl">
	<tr>
		<th>ФИО</th><th>Calculated</th><th>Validity</th>
...
|FIO|Calculated|Validity|RR_MD_Min|RR_MD_Max|RR_MD_Avg|RR_AM_Min|RR_AM_Max|RR_AM_Avg|BOI_MD_Min|BOI_MD_Max|BOI_MD_Avg|BOI_AM_Min|BOI_AM_Max|BOI_AM_Avg|
<!-- BEGIN row -->
|{row.FIO}|{row.CALCULATED}|{row.VALIDITY}|{row.RR_MD_MIN}|{row.RR_MD_AVG}|{row.RR_MD_MAX}|{row.RR_AM_MIN}|{row.RR_AM_AVG}|{row.RR_AM_MAX}|{row.BOI_MD_MIN}|{row.BOI_MD_AVG}|{row.BOI_MD_MAX}|{row.BOI_AM_MIN}|{row.BOI_AM_AVG}|{row.BOI_AM_MAX}|
<!-- END row -->
}}
</textarea>

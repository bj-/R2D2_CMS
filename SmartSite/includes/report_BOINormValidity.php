...
ORDER BY 
	[p].[Last_Name] ASC, [p].[First_Name] ASC, [p].[Middle_Name] ASC,
	[bn].[Calculated] DESC
";

	$SQL = ( $uGuid ) ? str_replace("%%GUID%%", "AND [u].[Guid] = '$uGuid'", $SQL) : str_replace("%%GUID%%", "", $SQL) ;
	...
	if ( $uGuid )
	{
		$template->set_filenames(array(
			'report' => 'incl/report_BOINormValidity.tpl')
		);
	}
	else
	{
		$template->set_filenames(array(
			'report' => 'incl/report_BOINormValidityAll.tpl')
		);
	}
